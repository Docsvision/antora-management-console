...
.Регистрация расширений аутентификации в {of-mc}
image::auth-extensions.png[Регистрация расширений аутентификации в {of-mc}]
...
+
При переходе на страницу настроек расширения аутентификации все несохранённые изменения будут применены. При наличии несохранённых изменений будет выведено соответствующее сообщение с кнопками *Ок* и *Отмена*. При нажатии *Ок* изменения будут применены, при нажатии *Отмена* -- отменены.
+
. На странице настроек расширения аутентификации представлены следующие поля:
+
* Идентификатор расширения -- строка с идентификатором расширения в виде Guid. Идентификатор генерируется автоматически случайным образом.
...
В таблице отображается список расширений, которые были добавлены и <<setup,настроены>> пользователем. Над таблицей отображается название базы данных. Для базы, которая назначена по умолчанию, добавляется тэг "БД по умолчанию".
