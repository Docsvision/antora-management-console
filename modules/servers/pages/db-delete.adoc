...
Если база данных {dv} больше не используется, после отключения от {dv} она может быть удалена из системы управления базами данных.
...
+
Для БД {mssql} Server воспользуйтесь SQL Server Management Studio.
+
Для {pgsql} воспользуйтесь dbForge Studio for {pgsql} или аналогичной программой.
+
...
END
----
<.> `DBName` замените на название БД {dv}, которая была удалена.

[NOTE]
====
include::6.1@platform:admin:partial$excerpts.adoc[tags=delete-wf-settings]
====
